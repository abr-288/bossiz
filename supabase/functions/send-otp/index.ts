...
// Limite par IP en plus du cooldown par destination ci-dessous : sans elle,
// un même appelant peut spammer un grand nombre de destinations différentes
// (coût SMS, harcèlement par email) puisque cette fonction n'exige qu'une clé
// anon, publique par nature.
const IP_RATE_LIMIT = { windowMs: 15 * 60 * 1000, maxRequests: 5 };
...
serve(async (req) => {
  ...
  const rateLimitResult = checkRateLimit(clientIP, { ...IP_RATE_LIMIT, keyPrefix: 'send-otp' });
