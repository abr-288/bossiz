...
import bannerImage from "@/assets/hotel-onomo.jpg";

const BossizCIServices = () => {
  ...
  if (loading || !content) {
    return (
      <BossizSiteLayout country="ci" activePage="services">
  ...
  return (
    <BossizSiteLayout country="ci" activePage="services" tagline={content.tagline}>
      {/* Bannière avec image de fond */}
      <section className="relative h-64 md:h-80 overflow-hidden">
        <img src={bannerImage} alt="Services Bossiz Conciergerie" className="absolute inset-0 w-full h-full object-cover" />
              ...
              return (
                <Card key={service.id} className="rounded-xl border-0 border-t-4 border-t-bossiz-gold shadow-md bg-white">
                  <CardContent className="p-8">
                    <div className="w-14 h-14 rounded-full bg-bossiz-ci-green-pale flex items-center justify-center mb-6">
                      <Icon className="w-6 h-6 text-bossiz-ci-green" strokeWidth={1.5} />
                    </div>
                    ...
                    <button
                      ...
                      className="text-xs font-semibold uppercase tracking-wider text-bossiz-ci-green hover:text-bossiz-gold-dark inline-flex items-center gap-1"
                    >
              ...
              return (
                <Card key={service.id} className="rounded-xl border border-bossiz-cream-taupe/40 bg-bossiz-cream-warm/30">
                  <CardContent className="p-6 flex items-start gap-4">
                    <div className="w-11 h-11 rounded-full bg-bossiz-ci-green-pale flex items-center justify-center shrink-0">
                      <Icon className="w-5 h-5 text-bossiz-ci-green" strokeWidth={1.5} />
                    </div>
                    <div>
...
export default BossizCIServices;
