...
import aboutImage from "@/assets/destination-city.jpg";

const BossizCIAPropos = () => {
  const navigate = useNavigate();
  const { content, loading } = useBossizMicrositeContent("ci");

  if (loading || !content) {
    return (
      <BossizSiteLayout country="ci" activePage="a-propos">
  ...
  return (
    <BossizSiteLayout country="ci" activePage="a-propos" tagline={content.tagline}>
      {/* Bannière avec image de fond */}
      <section className="relative overflow-hidden">
        <div className="absolute inset-0">
          <img src={aboutImage} alt="Bossiz Conciergerie Côte d'Ivoire" className="w-full h-full object-cover" />
          <div className="absolute inset-0 bg-gradient-to-b from-bossiz-ci-green/92 via-bossiz-ci-green/85 to-bossiz-ci-green/92" />
        </div>
...
export default BossizCIAPropos;
